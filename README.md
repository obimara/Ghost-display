...
`rustdesk-auto-display` keeps the current Wayland session when a physical DRM connector is connected and the `WAYLAND_DISPLAY` socket exists in `XDG_RUNTIME_DIR`. If Wayland is not available, it chooses `:0` when that X display responds to `xset`; otherwise it chooses the ghost display `:20`. Override with `RUSTDESK_PHYSICAL_DISPLAY`, `RUSTDESK_GHOST_DISPLAY`, `RUSTDESK_PREFER_WAYLAND=0`, or `RUSTDESK_WAYLAND_RUNTIME_DIR` if needed.
