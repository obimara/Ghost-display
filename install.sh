...
Installs Ghost-display dependencies, Xorg config, runtime scripts, and systemd service.
...
install_optional_script() {
  local source_path="$1"
  local target_path="$2"

  if [[ -f "${source_path}" ]]; then
    install -D -m 0755 "${source_path}" "${target_path}"
  fi
}
...
install_files() {
  ...
  install_optional_script "${ROOT_DIR}/scripts/debug-ghost-display.sh" /usr/local/bin/ghost-display-debug
}
...
activate_service() {
  if ! command -v systemctl >/dev/null 2>&1; then
    echo "systemctl not found; start manually with: /usr/local/bin/ghost-display-x11 --foreground" >&2
    return
  fi

  if [[ ! -d /run/systemd/system ]]; then
    echo "systemd is not active; start manually with: /usr/local/bin/ghost-display-x11 --foreground" >&2
    return
  fi
