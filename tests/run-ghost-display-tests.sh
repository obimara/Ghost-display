...
PY
    ...
    echo "Fake Wayland socket did not appear: ${socket_path}" >&2
    exit 1
}
...
fake_wayland_runtime="${TMP_DIR}/wayland-runtime"
mkdir -p "${fake_wayland_runtime}"
start_fake_wayland_socket "${fake_wayland_runtime}/wayland-1"
WAYLAND_DISPLAY=wayland-1 XDG_SESSION_TYPE=wayland XDG_RUNTIME_DIR="${fake_wayland_runtime}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" --print >"${TMP_DIR}/auto-wayland.out"
assert_contains "${TMP_DIR}/auto-wayland.out" "wayland:wayland-1"
WAYLAND_DISPLAY=wayland-missing XDG_SESSION_TYPE=wayland XDG_RUNTIME_DIR="${fake_wayland_runtime}" PATH="${MOCK_BIN}:${PATH}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" --print >"${TMP_DIR}/auto-wayland-stale.out"
assert_contains "${TMP_DIR}/auto-wayland-stale.out" ":0"
RUSTDESK_PREFER_WAYLAND=0 WAYLAND_DISPLAY=wayland-1 XDG_SESSION_TYPE=wayland XDG_RUNTIME_DIR="${fake_wayland_runtime}" PATH="${MOCK_BIN}:${PATH}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" --print >"${TMP_DIR}/auto-wayland-disabled.out"
assert_contains "${TMP_DIR}/auto-wayland-disabled.out" ":0"

WAYLAND_DISPLAY=wayland-1 XDG_SESSION_TYPE=x11 XDG_RUNTIME_DIR="${fake_wayland_runtime}" PATH="${MOCK_BIN}:${PATH}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" --print >"${TMP_DIR}/auto-wayland-x11-session.out"
assert_contains "${TMP_DIR}/auto-wayland-x11-session.out" ":0"
echo disconnected >"${fake_drm}/card0-HDMI-A-1/status"
WAYLAND_DISPLAY=wayland-1 XDG_SESSION_TYPE=wayland XDG_RUNTIME_DIR="${fake_wayland_runtime}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" --print >"${TMP_DIR}/auto-wayland-no-connector.out"
...
WAYLAND_DISPLAY=wayland-1 XDG_SESSION_TYPE=wayland XDG_RUNTIME_DIR="${fake_wayland_runtime}" PATH="${MOCK_BIN}:${PATH}" GHOST_TEST_LOG="${LOG_FILE}" RUSTDESK_DRM_DIR="${fake_drm}" "${ROOT_DIR}/scripts/rustdesk-auto-display.sh" mock-rustdesk
