...
DISPLAY_ID="$(safe_display_id)"
CONFIG_FILE="${GHOST_XORG_CONFIG:-/etc/X11/ghost-display.conf}"
PID_FILE="${XDG_RUNTIME_DIR:-/tmp}/ghost-display-${DISPLAY_ID}.pid"
...
xorg_pid_matches() {
    ...
    while IFS= read -r -d '' arg; do
        if [[ "${arg}" == "${DISPLAY_NAME}" ]]; then
            has_display=1
        fi
        if [[ "${previous}" == "-config" && "${arg}" == "${CONFIG_FILE}" ]]; then
            has_config=1
        fi
        previous="${arg}"
    done <"/proc/${pid}/cmdline"

    [[ "${has_display}" == "1" && "${has_config}" == "1" ]]
}

find_existing_xorg_pid() {
    local pid
    ...
    done < <(pgrep -x Xorg 2>/dev/null || true)
...
is_xorg_running() {
    ...
    pid="$(find_existing_xorg_pid || true)"
    if [[ -n "${pid}" ]]; then
        write_pid_file "${pid}"
        return 0
    fi
